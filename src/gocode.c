...
go_back_next(int direct)
{
	...
	if (is_typepds) {
		del_space(go.dir);
		if (strlen(go.dir)<1) {
			sprintf(go.dir,"pds/%s/%s",club_go.code,go.code);
			check_dir(go.dir);
		}
	}
	re_print=Yes;
...
go_code(char *buff)
{
	...
	_fail:
	if ((fp=fopen("system/findgo.dat","r"))!=NULL) {
		int found=No;
		char buff1[100],buff2[100],buff3[100];
		while (fgets(buff,200,fp)) {
			sscanf(buff,"%s%s",buff1,buff2);
			if (is_same(buff1,buffgo)&&strlen(buff2)) {
				sprintf(auto_cmd,"go %s %s",buff2,buff1);
				fclose(fp);
				return Yes;
			}
		}
...
mm_code(int mode)
{
	...
	if ((fp=fopen(filename,"r"))!=NULL) {
		...
		fclose(fp);
		if (mode>0&&mode<=100&&strlen(mm[mode-1])) strcpy(auto_cmd,mm[mode-1]);
	}
}
