...
{
    FILE *fp,*fp2;
    char buff[5000],buff1[1100],buff2[1100],buff3[1100];
    ...
    while (fgets(buff,5000,fp)) {
        buff[1000]=0;
        if (strlen(buff)<50||strlen(buff)>2500) continue;
        for (i=6;i<30;i++) {
            strncpy(buff1,&buff[i],6);
            if (strcmp(buff1," - - [")==0) {
                bzero(buff2,1100);strncpy(buff2,&buff[i+9],3);
                bzero(buff3,1100);strncpy(buff3,&buff[i+13],4);
