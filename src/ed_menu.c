...
//    printf("\n,%s,%s,\n",argv[1],argv[2]);
//    printf("\n## ¸Þ´º ÆíÁý ID:%s NAME:%s TYPE:%d CLUB:%s\n",pf.id,pf.name,pf.type,tclub);

    if (!find_club(tclub)) {mess("CLUB:%s not found",tclub);return;}
    sprintf(NOW_MENU ,"forum/%s/%s",club_go.code,MENU_FILE);
	check_topmenu();  //¸Þ´º0ÀÌ topÀÌ ¾Æ´Ï¸é topÀ¸·Î ÁöÁ¤
    check_dir("forum/%s",club_go.code);
    check_dir("forum/%s/system",club_go.code);
    check_dir("forum/%s/menustru",club_go.code);
    check_dir("forum/%s/menucfg",club_go.code);
    check_dir("forum/%s/menurec",club_go.code);

    sprintf(buff,"forum/%s/menustru/menu.dat",club_go.code);touch_file(buff);
    sprintf(NOW_MENU,"forum/%s/menustru/menu%d.old",club_go.code,now_day()%10);
	...
	while (1) {
        ...
        printf("[7m[2K ¸Þ´º:%-8s  ±íÀÌ:%-2d TOP:%d (Ç×¸ñ:%d %dK) %s[0;1m\n",
            subgo.code,now_deep,top,count_menu_num,file_size(NOW_MENU)/1024,all_view?"(ÀüÃ¼º¸±â)":"");
...
//		  if (linecount<16) canforwd=No;
        ...
        printf("ÀüÃ¼(G) ÀÌÀü(B) ´ÙÀ½(F) »óÀ§(P) Ãß°¡(A) »ðÀÔ(I) ÀúÀå(W) Á¤¸®(R)\n");
		...
		if (is_same(buff,"0")) nn=0;
			...
			check_menu();
			//¸Þ´ºÆÄÀÏ·Î ÀúÀå
            sprintf(buff,"forum/%s/menustru/menu.dat",club_go.code);
...
ed_menu2(int n)
{
	...
	if (n==0) {
		...
		if (pf.type>=17||go.type=='n'||go.type=='r') {
			...
			strcpy(buff,go.dir);
            dosinput(buff,80);check_han(buff);strlwr(buff);
			all_trim(buff);
        ...
        xy(15,14);get_str(bcfg.sysop,20);
        xy(25,14);get_str(bcfg.sysop1,20);
        xy(35,14);get_str(bcfg.sysop2,20);
        xy(45,14);get_str(bcfg.sysop3,20);
	}
	else if (n==13) {
...
sdisp_menu(int nn)
{
	...
	printf("\n");
	printf(" 11.Å° ¿ö µå: %s  \n",bcfg.key);
    printf(" 12.´ã´çÁöÁ¤: %-15s %-15s %-15s %-15s\n",
...
reset_menuind() 	//¸Þ´ºSUB TAG ÀçÁ¤¸®
{
    ...
    sys_tem("rm -f forum/%s/menustru/*.ind",club_go.code);
	if ((fp=fopen(NOW_MENU,"r"))==NULL) return;
	deep=0;
	n=-1;
	while(1) {
		...
		if (tgo.type=='m'&&strlen(tgo.code)>0) {
            sprintf(filename,"forum/%s/menustru/%s.ind",club_go.code,tgo.code);
...
sdn_menu()
{
	...
	if (yesno("\n ÇöÀçÀÇ ¸Þ´º¸¦ MENU.ZIPÈ­ÀÏ·Î ´Ù¿î¹ÞÀ¸½Ã°Ú½À´Ï±î? (y/N)>>",No)) {
        ...
        sprintf(buff,"forum/%s/menustru",club_go.code);
...
make_menu_index()
{
	...
	//¸ÞÀÎ ÀÐ±â
    sprintf(menu_ind,"menustru/menuind.tmp");
    ...
    sprintf(filename,"forum/%s/menustru/menu.dat",club_go.code);
	if ((fp=fopen(filename,"r"))==NULL) return;
