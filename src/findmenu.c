...
char *argv[];
{
    ...
    if (mode==0) {
        view("forum/main/frm/go/gosel.log");
		lineinput(buff,20);
		if (strlen(buff)<1) return;
        if (is_same(buff,"h")) {
            view("forum/main/frm/go/gohelp.log");
        }
        else if (is_same(buff,"0")) {
            find_menu(1);
        }
        else if (is_same(buff,"1")) {
            if (!view("forum/%s/frm/go/goindex.log",club_go.code))
                 view("forum/main/frm/go/goindex.log");
        }
        else {
            find_menu(3);        //검색
        }
...
find_menu(int mode)
{
	...
	while (1) {
        ...
        view("forum/main/frm/go/gosel.lst");
		printf(C_SAVE);
		xy(1,3);
...
find_menu2(int mode)
{
    ...
    sprintf(filename,"forum/%s/%s/menustru/menu.dat",club_go.code);
	if ((fp=fopen(filename,"r"))==NULL) {
		mess("%s 파일이 없습니다.",filename);
		return;
	}
...
gofind()
{
    ...
    strcpy(buff1,"");
    strcpy(buff2,"");
    view("forum/main/frm/go/gofind.scr");
...
view_ind(char *buff1,char *buff2)
{
    ...
    INDEXGO indexgo;
    strcpy(auto_cmd,"");
    strcpy(menu_ind,"forum/main/menustru/menuind.tmp");
    ...
    while (1) {
        ...
        sprintf(g_buff2,"(%d/%d)",(top/20)+1,(count/20)+1);
        view("forum/main/frm/go/gofind.lst");
        printf(C_SAVE);
        xy(1,3);
        for (i=top;i<top+20;i++) {
            printf("%3d. %s\n",i+1,abuff[i]);
...
make_manu_index()
{
    ...
    //메인 읽기
    sprintf(menu_ind,"forum/main/menustru/menuind.tmp");
    if ((fp2=fopen(menu_ind,"w"))==NULL) return;

    count=0;
    club_count=0;
    sprintf(filename,"forum/main/%s/menustru/menu.dat",club_go.code);
    ...
    fclose(fp);
    for (i=0;i<club_count;i++) {
        sprintf(filename,"forum/main/%s/menustru/menu.dat",aclub[i].club);
