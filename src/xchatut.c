...
invite_man(char *tid,char *buffstr)
{
	...
	if (strlen(toid)<3) {  //¾ÆÀÌµð ÁöÁ¤¾ÈµÊ
        if (sel_user("forum/main/frm/say/invite.mes",0))
			strcpy(toid,moni.id);
	}
	key=No;
	n=is_moni(toid);
    ...
    if (to_moni(moni,0,messbuff)) printf(" ## [%s]´Ô¿¡°Ô ÃÊ´ë¸Þ¼¼Áö¸¦ º¸³Â½À´Ï´Ù.\n",toid);
...
save_croom()
{
	...
	strchng2(croom.stitle,"\'","\\\'");strchng2(croom.stitle,"\"","\\\"");
    text_cut(croom.id,12);text_cut(croom.name,12);text_cut(croom.boss2,12);
    text_cut(croom.title,150);text_cut(croom.stitle,60);text_cut(croom.ctitle,20);
