///////////////////////////////////////////////////////////////////////////
// XHOST: ∏Æ¥™Ω∫øÎ BBS HOST  ∞≥πﬂ:±Ëº∫¥Î(053-964-1002) BBS:053-963-2190
// √§∆√ µ∂∏≥«¡∑Œ±◊∑•
///////////////////////////////////////////////////////////////////////////
//√ﬂ∞°±‚¥…
// «œ¥‹ ¡Ÿº±≈√,ªˆ±Ú∏∂¿Ω¥Î∑Œ º±≈√∞°¥…
...
do_chat(int mode)
{
	...
	if (mode==2&&club_mode&&!is_same(club_go.code,"main")) {
        roomnum=9500+ccfg.roomnum%500;
	}
    else if (strncmp(go.code,"chat_",5)==0) {
        strcpy(buff,&go.code[5]);
        n=atoi(buff);
        if (n>0) roomnum=n;
	}

	if (pf.type<cfg.chattype) {
        if (!view("forum/main/frm/error/chatoff.log")) mess("¥Î»≠Ω«¡¯¿‘±›¡ˆ");
		return;
	}
	...
	//¡¯¿‘»≠∏È√‚∑¬
    if (!view("forum/main/frm/chat/chat%d.log",roomtype))
         view("forum/main/frm/chat/chat.log");

    st_mode=cfg.chatst;
    if (roomnum>0) {  //πÊπ¯»£ πŸ∑Œ∞°±‚
        join_directroom();
    }
    else {
        cont_chat=Yes;
...
select_room()
{
    ...
    if (n>=9500&&n<9900) {
        ...
        if (!view("forum/main/frm/chat/chat%s.hlp",mmc[1]))
            view("forum/main/frm/chat/chat.hlp");
		goto _cont1;
	}
...
int join_newroom()
{
    ...
    view("forum/main/frm/chat/copen2.log");
	time(&croom.date);
	croom.roomtype=roomtype;
	xstrncpy(croom.id,userid,20);
...
int join_directroom()
{
	...
	check_same();		//∞∞¿∫ ID∞° ¿÷¥¬∞°?
	add_croom();
    if (roomnum>=9500&&roomnum<=9900) sprintf(croom.title,"[µø»£»∏] %s",ccfg.name);
	save_croom();
	check_newboss();
	chatstart();		 //¥Î»≠Ω√¿€
	chat_title(1);
...
chatstart()
{
    ...
    view("forum/main/frm/chat/chatin.mes");
...
//		  printf("\n %d %s %d %d\n",roomnum,ROOM_FILE,file_time(ROOM_FILE),file_size(ROOM_FILE));
		...
		if (strlen(chatbuff)<1) {
            ...
            if (roomtype!=7&&!view("forum/main/frm/chat/chatad.%d",rand()%50))
                view("forum/main/frm/chat/chatad.0");
			continue;
		}
		if (mynum>0) {
			time(&room.ctime);
...
chat_title(int mode)
{
    ...
    sprintf(filename,"forum/main/frm/chat/chat%d.top",roomtype%100);
	if (file2buff(buff,filename,3000)<0) {
        strcpy(filename,"forum/main/frm/chat/chat1.top");
...
disp_room1()
{
    ...
    //¥Î±‚Ω«
    sprintf(buff2,"forum/main/frm/chat/stwait0.log");
    ...
    sprintf(buff2,"forum/main/frm/chat/stwait%d.log",st_mode);
	file2buff(buff,buff2,0);
	fprintf(fp,buff);
...
*/
        ...
        sprintf(buff2,"#%3d",croom.roomnum);
	...
	strcpy(auto_cmd,"");
    if (!view("forum/main/frm/chat/st-%d.scr",roomtype))
        view("forum/main/frm/chat/st.scr");
	if ((fp=fopen(st_file,"r"))!=NULL) {
		while (fgets(buff,2000,fp)!=NULL) {
			printf("%s",buff);
			if (count++%20==19) {
                if (!view("forum/main/frm/chat/st.cmd"))
...
add_croom()
{
	...
	for (i=0;i<CMAX;i++) {
		if (strlen(user_room[i].id)<2) {
			room=user_room[i];
            text_cut(userid,12);text_cut(userna,12);text_cut(userni,12);
            xstrncpy(room.id  ,userid,12);text_cut(room.id,12);
            xstrncpy(room.name,userna,12);text_cut(room.name,12);
            xstrncpy(room.nick,userni,12);text_cut(room.nick,12);
			strcpy(room.tty ,tty);
