...
char *argv[];
{
	...
	strcpy(go.code,argv[2]);
	read_ccfg();
	read_bcfg();

    if (!is_same(argv[3],"root")&&!read_pf(argv[3])) {
        printf("\n %s 회원이 없습니다.\n",argv[3]);
		pressenter();
		return;
	}
	...
	mysql_close(sock);
    if (argc>4&&is_same(argv[4],"__conv1")) {old_new(2);return;}
	...
	while (1) {
		...
		cls();
		print_(1,"%s[%s] %s (%d)",club_go.code,go.code,ccfg.name,NOW_ALLNO);
        ...
        if (is_same(pf.id,"sysop")) {
            printf(" 97. XHOST 변환   98. KOMA 변환\n");
            printf(" 99. 기타형식(XFORMAT,버젼별) 파일 변환\n");
        }
		printf(" 도움(H)  종료(P)  선택 >>");
        ...
        else if (mode==97&&is_same(pf.id,"sysop")) conv_xhost();
        else if (mode==98&&is_same(pf.id,"sysop")) conv_koma();
        else if (mode==99&&is_same(pf.id,"sysop")) conv_bbs();
...
cho_bbs2(char *filename)
{
	...
	sprintf(bbs.keyword,"`%7.7s",buff3);
...
edit_title()
{
	...
	while (fread(&bbs,sizeof_bbs,1,fp1)) {
		sprintf(buff,"%c%7d %-8s %-8s %-12s %s\n",
            bbs.del=='D'?'D':' ',bbs.num,bbs.id,bbs.name,bbs.filename,bbs.title);
		fprintf(fp2,buff);
	}
	fclose(fp1);
	fclose(fp2);
...
bbs_bbs()
{
	...
	if (stristr(buff,"user/")) goto _fail;
	if (stristr(buff,"/etc/")) goto _fail;
...
//		   제  목:[RA]강추! EVA! '무한포옹' 입니다.        분류:SDATA    전송:310    1/ 1
			...
			strchng8(buff2,"분류:");
			strchng2(buff2,"분류:","");
			xstrncpy(bbs.keyword,buff2,8);
			all_trim(bbs.keyword);
			if (get_line(buff)==No) break;	//1줄 입력
			strcpy(buff2,buff);
			strchng9(buff2,"크기:");
			strchng2(buff2,"파일명:","");
...
// mode==0: 일반  2:메일데이타   drop:2 삭제
xformat1(char *def_file,char *table_name,int mode,int drop)
{
	FILE *fp1;
	char buff[1010];
	char buff1[100010];
	...
	if (mode==2) {
		if (strlen(pf.id)<1||strlen(pf.id)>30) return;
...
//		  if (bbs.num>=0) {printf(" B1*%s*%s B2*%s*",buff1,buff,buff2);pressenter();}
		...
		else if (is_same(buff,"__[[완료[[")||strlen(body)>=64000) {
			found=Yes;
			strcpy(cid,bbs.id);strcpy(cname,bbs.name);

			if (strlen(bbs.id)||strlen(bbs.name)||strlen(bbs.title)||strlen(bbs.filename)) {
                if (mode==2&&strlen(body)<4&&strlen(bbs.filename)<1) {
                    printf("%s : %s(%s) %s ->내용없음\n",table_name,bbs.id,bbs.name,bbs.title);
                }
                else {
                    printf("%s : %s(%s) %s\n",table_name,bbs.id,bbs.name,bbs.title);
		...
		else if (is_same(buff,"__[[제목[[")) xstrncpy(bbs.title,buff2,70);
		else if (is_same(buff,"__[[아이[[")) xstrncpy(bbs.id   ,buff2,20);
		else if (is_same(buff,"__[[이름[[")) xstrncpy(bbs.name ,buff2,20);
		else if (is_same(buff,"__[[TID [[")) xstrncpy(bbs.tid  ,buff2,20);
		else if (is_same(buff,"__[[키워[[")) xstrncpy(bbs.keyword,buff2,30);
		else if (is_same(buff,"__[[주석[[")) xstrncpy(bbs.note,buff2,100);
		else if (is_same(buff,"__[[답변[[")) bbs.an=atoi(buff2);
		else if (is_same(buff,"__[[파일[[")) xstrncpy(bbs.filename,buff2,30);
		else if (is_same(buff,"__[[FCOU[[")) bbs.fcount=atoi(buff2);
		else if (is_same(buff,"__[[파크[[")) bbs.filesize=atoi(buff2);
		else if (is_same(buff,"__[[티코[[")) xstrncpy(bbs.tcode,buff2,12);
		else if (is_same(buff,"__[[TCOD[[")) xstrncpy(bbs.tcode,buff2,12);
		else if (is_same(buff,"__[[티클[[")) xstrncpy(bbs.tclub,buff2,12);
		else if (is_same(buff,"__[[TCLU[[")) xstrncpy(bbs.tclub,buff2,12);
		else if (is_same(buff,"__[[날짜[[")) bbs.date=str2date2(buff2);
		else if (is_same(buff,"__[[등록[[")) bbs.date=str2date2(buff2);
		else if (is_same(buff,"__[[조날[[")) bbs.date2=str2date2(buff2);
		else if (is_same(buff,"__[[번호[[")) bbs.num=atoi(buff2);
		else if (is_same(buff,"__[[줄수[[")) bbs.line=atoi(buff2);
		else if (is_same(buff,"__[[다운[[")) bbs.down=atoi(buff2);
		else if (is_same(buff,"__[[조회[[")) bbs.hits=atoi(buff2);
		else if (is_same(buff,"__[[라인[[")) bbs.line=atoi(buff2);
		else if (is_same(buff,"__[[찬성[[")) bbs.ok=atoi(buff2);
		else if (is_same(buff,"__[[추천[[")) bbs.good=atoi(buff2);
        else if (is_same(buff,"__[[삭제[[")) {
                if (atoi(buff2)==68) bbs.del='D';
                else bbs.del=buff2[0];
        }
		else if (is_same(buff,"__[[내용[[")) {
			bzero(body,BODY_MAX);
			text_mode=Yes;
		}
		else if (text_mode) {
...
xformat2()	//xformat으로 변환
{
	...
	while (fread(&bbs,sizeof_bbs,1,fp1)){
		...
		fprintf(fp3,"__[[주석[[%s\n",bbs.note);
		fprintf(fp3,"__[[줄수[[%d\n",bbs.line);
        if (bbs.del           =='D') fprintf(fp3,"__[[삭제[[%c\n",bbs.del);
		if (bbs.hits			!=0) fprintf(fp3,"__[[조회[[%d\n",bbs.hits);
		if (bbs.down			!=0) fprintf(fp3,"__[[다운[[%d\n",bbs.down);
		if (bbs.good			  !=0) fprintf(fp3,"__[[추천[[%d\n",bbs.good);
		if (bbs.ok				!=0) fprintf(fp3,"__[[찬성[[%u\n",bbs.ok);
		if (bbs.no				!=0) fprintf(fp3,"__[[반대[[%u\n",bbs.no);
		...
		if (strlen(bbs.keyword		)>0) fprintf(fp3,"__[[키워[[%s\n",bbs.keyword);
...
add_table(int mode,MYSQL *sock,char *table_name,char *body)
{
	...
	addslashes(bbs.title,sizeof(bbs.title));
	addslashes(bbs.id,sizeof(bbs.id));
	addslashes(bbs.tcode,sizeof(bbs.tcode));
	addslashes(bbs.homepage,sizeof(bbs.homepage));
	addslashes(bbs.name,sizeof(bbs.name));
	addslashes(bbs.keyword,sizeof(bbs.keyword));
	...
	bbs.hits,bbs.down,bbs.ok,bbs.no,bbs.good,
